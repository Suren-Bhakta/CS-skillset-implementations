...
class Graph(object):
    ...
    def review_mat(self, stack):
        y = len(self.Vertices)
        vert = stack.peek()
        for i in range(y):
            visited = self.Vertices[i].was_visited()
            edge = self.adjMat[vert][i]
    ...
    def has_cycle(self):
        # This implementation will be done using the code written above for dfs search
        # It was attempted to use dfs directly but time limit was exceeded for some arbitrary reason
        # Thus we were forced to rewrite the dfs by hand in this function with helper functions
        for v in range(len(self.Vertices)):
            theStack = Stack()
            self.Vertices[v].visited = True
            theStack.push(v)
            while not theStack.is_empty():
                u = self.review_mat(theStack)
                if u == -1:
                    u = theStack.pop()
                elif u == 0: # This elif method was created which prevented the time limit issue
    ...
    def bfs(self, v):
        ...
        while (not q.is_empty()):
            # get an adjacent unvisited vertex
            uv = self.get_adj_unvisited_vertex(q.current())
            if (uv == -1):
                uv = q.dequeue()
            else:
                (self.Vertices[uv]).visited = True
                print(self.Vertices[uv])
                q.enqueue(uv)
        # the stack is empty, let us reset the flags
        new_vert = len(self.Vertices)
        for i in range(new_vert):
            (self.Vertices[i]).visited = False
    ...
    def toposort(self):
        topoList = []
        theGraph = Graph()
        theGraph.Vertices = self.Vertices[:]
        theGraph.adjMat = self.adjMat[:][:]
        while len(theGraph.Vertices) != 0:
            lst=[]
            # this for loop appends all the vertecis after the sort is performed
            for vertex in theGraph.Vertices:
                u = theGraph.get_index(vertex.label)
                if len(theGraph.all_vert(u)) == 0: # KEY BOOLEAN
                    lst.append(vertex.label)
            
            #this set of code will sort the vertices alphabetically
            lst.sort()
            topoList.append(lst)
            for label in lst:
                theGraph.delete_vertex(label)
